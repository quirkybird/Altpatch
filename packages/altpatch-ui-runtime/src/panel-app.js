...
function loadStoredDock() {
    const storage = getLocalStorage();
    const raw = storage?.getItem(STORAGE_DOCK_KEY);
    if (raw === 'float' || raw === 'bottom' || raw === 'left' || raw === 'right')
        return raw;
    return 'float';
}
...
function resolveSourceLocation(target) {
    const normalizeLocatorPath = (input) => {
        const raw = String(input ?? '').trim().replace(/^['"]|['"]$/g, '');
        if (!raw)
            return '';
        return raw.startsWith('/@fs/') ? raw.slice('/@fs/'.length) : raw;
    };
    const resolveByReactDebugSource = () => {
        const renderers = window.__REACT_DEVTOOLS_GLOBAL_HOOK__?.renderers;
        const list = renderers ? Array.from(renderers.values()) : [];
    ...
    if (runtime?.locate) {
        ...
        if (resolved) {
            const filePath = normalizeLocatorPath(resolved.filePath);
            if (filePath) {
                return {
                    ...resolved,
                    filePath
...
function snippetFromSource(source, line, range = 8) {
...
function buildSnippetNeedles(selectedText) {
...
function resolveDisplayLineForSnippet(source, anchorLine, selectedText) {
    const lines = source.split('\n');
    if (lines.length === 0)
        return 1;
...
function buildQuickTextAfter(source, line, oldText, nextText, notFoundMessage) {
...
function renderDiff(diffLines, emptyLabel) {
...
function wantsMultiFileMode(instruction) {
...
function renderHighlighted(code, mode = 'code') {
...
function clipText(input, maxLength = 46) {
    const value = input.trim().replace(/\s+/g, ' ');
    if (value.length <= maxLength)
        return value;
    return `${value.slice(0, maxLength)}...`;
}
function buildHistoryTitle(mode, inputValue) {
    const prefix = mode === 'quick' ? 'Quick' : 'AI';
    const content = inputValue.trim().length > 0 ? clipText(inputValue) : '(empty)';
    return `${prefix}: ${content}`;
}
function formatHistoryMeta(filePath, line, column, createdAt) {
...
function joinPosix(baseDir, segment) {
...
async function collectRelatedFilesForPrompt(apiPrefix, filePath, source) {
    ...
    for (const spec of specs) {
        ...
        for (const candidate of candidates) {
            ...
            seen.add(candidate);
            try {
                const data = await postJson(`${apiPrefix}/read-file`, { filePath: candidate });
                related.push({
                    filePath: candidate,
...
function dockStyle(dock, size, pos, isFullscreen) {
    if (isFullscreen) {
        return {
            position: 'fixed',
            inset: 0,
...
const AltPatchApp = ({ apiPrefix }) => {
    ...
    const multiApplyFilesRef = useRef([]);
    const diffRef = useRef([]);
    const selectedTextRef = useRef('');
    const hoverOverlayRef = useRef(null);
    const altPressedRef = useRef(false);
    const lastPointerRef = useRef(null);
    const [dock, setDock] = useState(() => loadStoredDock());
    ...
    const setStatusSafe = useCallback((message) => setStatus(message), []);
    ...
    useEffect(() => {
        ...
        setCodeHtml((prev) => Object.values(PANEL_I18N).some((x) => x.selectElementWithAltClick === prev) ? texts.selectElementWithAltClick : prev);
    ...
    useEffect(() => {
        ...
        if (!storage)
            return;
        storage.setItem(STORAGE_SIZE_KEY, JSON.stringify({
    ...
    const stopDrag = useCallback(() => {
        if (!dragState.current.active && dragState.current.pointerId === null)
            return;
    ...
    const stopResize = useCallback(() => {
        if (!resizeState.current.active && resizeState.current.pointerId === null)
            return;
    ...
    const handleSetDock = useCallback((next) => {
    ...
    const loadSource = useCallback(async (location) => {
        setStatusSafe(texts.readingSource);
        const data = await postJson(`${apiPrefix}/read-file`, { filePath: location.filePath });
    ...
    const handleGenerate = useCallback(async () => {
        ...
        if (mode === 'quick') {
            ...
            try {
                ...
                const diffData = await postJson(`${apiPrefix}/diff`, {
        ...
        setStreamOutputRaw('');
        try {
            const relatedFiles = await collectRelatedFilesForPrompt(apiPrefix, locationRef.current.filePath, sourceRef.current);
            if (wantsMultiFileMode(instruction)) {
                const targetFilePaths = relatedFiles.slice(0, 2).map((item) => item.filePath);
                const multi = await postJson(`${apiPrefix}/modify-multi`, {
                ...
                const okResults = multi.results.filter((item) => item.result);
            ...
            const runStreamModify = async (scopeMode) => {
                const body = {
            ...
            try {
                ...
                const maybeScopeError = error;
                if (maybeScopeError.code !== 'SCOPE_ERROR')
                    throw error;
                setStatusSafe(texts.scopeFull);
                setStreamOutputRaw('');
    ...
    const handleApply = useCallback(async () => {
        if (multiApplyFilesRef.current.length > 0) {
            ...
            try {
                await postJson(`${apiPrefix}/write-files`, {
        ...
        setStatusSafe(texts.applyingPatch);
        try {
            await postJson(`${apiPrefix}/write-file`, {
                filePath: location.filePath,
                content: nextContent
            });
            const snapshot = {
    ...
    const handleRestoreHistory = useCallback(async (id) => {
    ...
    const handleJumpToEditor = useCallback(async () => {
        ...
        try {
            await postJson(`${apiPrefix}/open-in-editor`, {
                filePath: locationRef.current.filePath,
                line: locationRef.current.line,
                column: locationRef.current.column
            });
    ...
    const handleAltClick = useCallback((event) => {
        ...
        if (dock === 'float' && !isFullscreen) {
            const placement = computePanelPlacementFromPointer({ x: event.clientX, y: event.clientY }, panelSize, { width: window.innerWidth, height: window.innerHeight });
    ...
    const handleResizeStart = useCallback((event, direction) => {
        if (isFullscreen)
            return;
        const allowedDirections = dock === 'float' ? new Set(['n', 's', 'w', 'e', 'nw', 'ne', 'sw', 'se']) :
    ...
    const historyItems = useMemo(() => historyEntries.map((entry) => ({ id: entry.id, title: entry.title, meta: entry.meta })), [historyEntries]);
    return (_jsx(Panel, { dock: dock, mode: mode, theme: theme, themeOptions: themeOptions, locale: locale, localeOptions: PANEL_LOCALE_OPTIONS, texts: texts, status: status, meta: meta, modelName: modelName, toast: toast, codeHtml: codeHtml, diffHtml: diffHtml, streamOutput: renderHighlighted(streamOutputRaw || '', 'code'), historyItems: historyItems, inputValue: inputValue, applyDisabled: applyDisabledFlag, undoDisabled: undoDisabledFlag, visible: visible, panelStyle: panelStyle, panelRef: panelRef, headerRef: headerRef, onDragStart: handleDragStart, onResizeStart: handleResizeStart, onClose: () => setVisible(false), onJumpToEditor: handleJumpToEditor, onSetDock: handleSetDock, isFullscreen: isFullscreen, onToggleFullscreen: handleToggleFullscreen, onSwitchMode: handleModeSwitch, onSetTheme: setTheme, onSetLocale: setLocale, onInputChange: handleInputChange, onGenerate: handleGenerate, onApply: handleApply, onUndo: handleUndo, onRestoreHistory: handleRestoreHistory, onDiscard: handleDiscard }));
};
function mountIntoShadow(apiPrefix) {
    ...
    root.render(_jsx(AltPatchApp, { apiPrefix: apiPrefix }));
}
export function mountAltPatchRuntime(apiPrefix = '/api') {
    if (typeof window === 'undefined')
        return;
    mountIntoShadow(apiPrefix);
}
//# sourceMappingURL=panel-app.js.map
